...
const Enrollment = () => {
  ...
  const [studentId] = useState('12345'); // Replace with actual student ID from auth context
  const [program] = useState('NS'); // Replace with actual program from user data
  const [semester] = useState('1'); // Replace with current semester
  const [courses, setCourses] = useState([]);
  const [lockedCourses, setLockedCourses] = useState(new Set());
  const [enrolledCourses, setEnrolledCourses] = useState([]);
  const [droppedCourses, setDroppedCourses] = useState([]);
  ...
  const [programAudit, setProgramAudit] = useState(null);
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [
          ...
          getProgramAudit(studentId),
          getTimetables(studentId, semester)
        ]);

        // Set state with all responses
        setEnrolledCourses(enrolledResponse.data);
        setDroppedCourses(droppedResponse.data);
        setCourses(availableResponse.data);
        setPrerequisites(prereqsResponse.data);
        setAcademicRecords(academicResponse.data);
        setProgramAudit(auditResponse.data);
        setTimetable(timetableResponse.data);

        // Check prerequisites for available courses
        const lockedSet = new Set();
        for (const course of availableResponse.data) {
          const prereqs = prereqsResponse.data[course.courseCode] || [];
          let hasUnmetPrereqs = false;
          
          if (course.courseCode === 'CS324') {
            // OR condition: at least one prerequisite must be met
            hasUnmetPrereqs = !prereqs.some(prereq =>
              enrolledResponse.data.some(ec => ec.courseCode === prereq)
            );
          } else if (course.courseCode === 'IS333') {
            // AND condition: all prerequisites must be met
            hasUnmetPrereqs = !prereqs.every(prereq =>
            ...
            // Default: any unmet prerequisite locks the course
            hasUnmetPrereqs = prereqs.some(
              prereq => !enrolledResponse.data.some(ec => ec.courseCode === prereq)
            );
          }
        ...
        console.error('[Enrollment] Error fetching data:', err);
  ...
  const confirmEnroll = async () => {
    ...
    try {
      const enrollmentDto = { 
        ...
        semester, // Added semester to enrollment data
        academicYear: new Date().getFullYear() // Add current academic year
      };
      
      // Call the enrollment endpoint
      const response = await enrollInCourse(enrollmentDto);
      
      // Update state with the new enrollment
      setEnrolledCourses((prev) => [...prev, { 
        ...selectedCourse, 
        status: response.data.status || 'Pending Approval' 
      }]);
      
      // Remove the enrolled course from available courses
      setCourses((prev) => prev.filter((c) => c.courseCode !== selectedCourse.courseCode));
      
      // Update the timetable if needed
      if (timetable) {
        const updatedTimetable = await getTimetables(studentId, semester);
      ...
      console.error('[Enrollment] Error enrolling course:', err);
      setError(err.response?.data?.message || `Failed to enroll in ${selectedCourse.courseName}. Please try again.`);
    } finally {
      setEnrollLoading(false);
      setEnrollDialogOpen(false);
      setSelectedCourse(null);
  ...
  const confirmDrop = async () => {
    ...
    try {
      ...
      console.error('[Enrollment] Error dropping course:', err);
      setError(err.response?.data?.message || `Failed to drop ${selectedCourse.courseName}. Please try again.`);
    } finally {
      setDropLoading(false);
      setDropDialogOpen(false);
      setSelectedCourse(null);
