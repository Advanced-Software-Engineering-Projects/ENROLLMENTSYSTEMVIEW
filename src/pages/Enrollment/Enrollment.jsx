...
//                           key={course.courseCode}
//                           value={course.courseCode}
//                           disabled={lockedCourses.has(course.courseCode) || (course.isActive === false)}
//                         >
//                           {course.courseCode} - {course.courseName}
//                           {lockedCourses.has(course.courseCode) && ' (Prerequisites not met)'}
//                         </MenuItem>
...
//                       <MenuItem key={`${course.courseCode}-${index}`} value={course.courseCode}>
//                         {course.courseCode} - {course.courseName} (Status: {course.status})
//                       </MenuItem>
//                     ))
//                   ) : (
...
const Enrollment = () => {
  ...
  // Check for student holds on component mount
  useEffect(() => {
    const checkHolds = async () => {
      try {
        ...
        if (userString) {
          ...
          const cleanId = user.email.split('@')[0];
          if (cleanId) {
            const holdsResponse = await getStudentWithHolds(cleanId);
            console.log("Holds Response:", holdsResponse.data);
            if (holdsResponse.data.holds.length > 0) {
              setHoldMessage(
                'Your account has holds preventing enrollment. Please contact the registrar to resolve.'
              );
        ...
        console.error('Error checking holds:', err);
        setHoldMessage('Failed to verify account status. Please try again later.');
        setHoldDialogOpen(true);
        setLoading(false);
      }
    };
  ...
  // Fetch enrolled and dropped courses
  useEffect(() => {
    if (!studentId || holdDialogOpen) return;
    ...
    fetchCourses();
  }, [studentId, program, holdDialogOpen]);
  ...
  return (
    <DashboardLayout>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle id="hold-dialog-title">Enrollment Hold</DialogTitle>
        <DialogContent>
