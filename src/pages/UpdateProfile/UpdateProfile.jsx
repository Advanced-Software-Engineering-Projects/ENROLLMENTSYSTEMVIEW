...
const StudentProfile = () => {
  const [editable, setEditable] = useState(false);
  const [student, setStudent] = useState(null); // Initially null, populated by API
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        ...
        setStudent(response.data); // Assuming response.data contains student object
        setLoading(false);
      } catch (err) {
  ...
  const handleEditToggle = async () => {
    if (editable) {
      ...
      try {
        ...
        setEditable(false);
        // Optionally, add a success message here (e.g., using a snackbar)
      } catch (err) {
  ...
  const handleAvatarChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        setStudent({ ...student, avatar: response.data.avatarUrl }); // Assuming response provides new URL
      } catch (err) {
            ...
            {[
              ...
              <Grid item xs={12} sm={6} key={field}>
                <TextField
                  ...
                  label={label}
                  name={field}
                  value={student[field] || ''} // Handle null/undefined values
                  onChange={handleChange}
                  disabled={disabled || !editable}
                  InputProps={{
                    startAdornment: (
